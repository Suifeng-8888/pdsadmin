...
<div class="hniois-table">
    ...
    <div class="hniois-table-header">
        <div class="hniois-search-btns">
            <c:if test="${fn:contains(QX,'plan/add')}">
            ...
            <c:if test="${fn:contains(QX,'plan/delete')}">
            ...
            <c:if test="${fn:contains(QX,'plan/approve')}">
            <a id="hniois-add" onclick="doApprove('0')" class="layui-btn layui-btn-sm layui-bg-green" href="javascript:;">
                <i class="fa fa-lg" aria-hidden="true"></i> 审批通过</a>
            <a id="hniois-add" onclick="doApprove('1')" class="layui-btn layui-btn-sm layui-bg-green" href="javascript:;">
                <i class="fa fa-lg" aria-hidden="true"></i> 审批驳回</a>
            </c:if>
        </div>
        <div class="hniois-search-inputs">
            <div class="hniois-search-keyword">
                <input type="text" class="layui-input hniois-search-data" name="bz" placeholder="采购批次号" />
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'plan/update')}">
...
<script>
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "plan/toEdit.do?id="+id+"&code="+code,
            area: ['750px', '80%']
        });
    }

    //送检弹窗
    function doApprove(sta){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].status!="2"){
                layui.layer.msg('选择的第'+(i+1)+'个数据已审批');
                return;
            }
            ids.push(data[i].id);
        }

        layui.layer.confirm('确认审批', function(){
            $.ajax({
                url: 'plan/approve.do',
                data: {
                    data:ids.toString(),
