...
<div class="hniois-table">
    <form action="plan/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
    <div class="hniois-table-header">
        <div class="hniois-search-btns">
            <c:if test="${fn:contains(QX,'approve/delete')}">
            ...
            <c:if test="${fn:contains(QX,'approve/price')}">
            <a id="hniois-add" onclick="doAskPrice()" class="layui-btn layui-btn-sm layui-bg-green" href="javascript:;">
                <i class="fa fa-lg" aria-hidden="true"></i> 去问价</a>
            </c:if>
        </div>
    ...
    <div class="hniois-search-mored layui-anim layui-anim-upbit">
        <div class="hniois-search-body">
            <div class="layui-form-item">
                <label class="layui-form-label">申批人</label>
                <div class="layui-input-block">
                    <input type="text" name="approver" autocomplete="off" class="layui-input hniois-search-data">
                </div>
            </div>
            <div class="layui-form-item">
                <label class="layui-form-label">询价情况</label>
                <div class="layui-input-block">
                    <select name="state"  lay-search="" class="hniois-search-data">
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
               ...
               //{field:'id',  title:'序号',sort: true},
                {field:'pur_batch',  title: '批次号',edit:'text'},
                {field:'pro_id', title: '产品编号'},
                {field:'pro_name',  title: '产品名称',edit:'text'},
                //{field:'pro_unit',  title: '产品规格',edit:'text'},
                {field:'pur_num', title: '购买数量',edit:'text'},
                {field:'approver',  title: '审批人',edit:'text'},
                {field:'approve_time',  title: '审批时间',edit:'text'},

                {field:'state',  title: '询价情况',templet:function(d){
                        if(d.state==0){
                            return "已询价";
                        }
                        if(d.state==1){
        ...
        table.on('edit(lay_table)', function(obj){
            if("${QX}".indexOf("approve/update")!=-1){
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "approve/toEdit.do?id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    function doAskPrice(){
        ...
        if (d.data.length === 0) {
            layui.layer.msg('请选择要询价的批次');
            return;
        }
        if(d.data.length>1){
            layui.layer.msg('每次请选择一个批次询价');
            return;
        }
        var data = d.data,
            id=data[0].id;
        if(data[0].state=="0"){
            layui.layer.msg('选择的数据已询价');
            return;
        }
        layer.open({
            title : "采购询价信息",
            ...
            maxmin: true,
            skin: 'layui-layer-rim', //加边框
            content: "approve/price.do?id="+id,
            area: ['750px', '80%']
        });
