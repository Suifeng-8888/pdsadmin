...
<form class="layui-form layui-form-pane" action="" >
    <input type="hidden" name="id" id="key" value="${produceSite.id}">
        <label class="layui-form-label">生产地址:<span style="color: #FF5722">*</span></label>
        <div class="layui-input-block">
            <input name="site" class="layui-input" type="text" autocomplete="off" lay-verify="required" value="${produceSite.site}">
        </div>
...
<script>
    ...
    layui.use(['layer', 'form'], function() {
        ...
        form.on('submit(formSave)', function(data){
            var url = "produce_site/" + ($("#key").val()==""?"add":"update") + ".do";
            $.ajax({
                url: url,
                data: data.field,
                type: "post",
