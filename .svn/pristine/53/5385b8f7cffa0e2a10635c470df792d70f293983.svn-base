...
<div >
    <form class="layui-form layui-form-pane" action="" >
        <input type="hidden" name="id" id="key" value="${r"${"}${packageName}.id${r"}"}">
<#list fieldList as e>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input name="${e.name}" class="layui-input" type="text" autocomplete="off"
                       lay-verify="required" value="${r"${"}${packageName}.${e.name}${r"}"}">
            </div>
        </div>
</#list>
...
<script>
    ...
    layui.use(['layer', 'form','treeselect'], function() {
        ...
        form.on('submit(formSave)', function(data){
            var url = "${packageName}/" + ($("#key").val()==""?"add":"update") + ".do";
            $.ajax({
                url: url,
                data: data.field,
                type: "post",
