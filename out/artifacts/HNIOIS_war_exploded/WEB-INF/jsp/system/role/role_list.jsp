...
<div class="hniois-table">
    <form action="role/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${fn:contains(QX,'role/add')}">
            ...
            <div class="hniois-search-inputs">
                <div class="hniois-search-keyword">
                    <input type="text"name="role_name"  class="layui-input hniois-search-data"  placeholder="搜索角色名称.." />
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'role/update')}">
        <a class="layui-btn layui-btn-xs" lay-event="edit">编辑</a>
    </c:if>
    <c:if test="${fn:contains(QX,'role/delete')}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        table.on('tool(lay_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                toEdit(data.role_id,0)
            }  else if(layEvent === 'edit'){
                toEdit(data.role_id,1)
            }else if(layEvent === 'del'){
                layer.confirm('确认删除么', function(index){
                    layer.close(index);
                    obj.del(); //删除对应行（tr）的DOM结构
                    //向服务端发送删除指令
                    doDelete(data.role_id);
                });
            }
        });
        ...
        table.on('edit(lay_table)', function(obj){
            if("${fn:contains(QX,'role/update')}"=="true"){
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "role/toEdit.do?role_id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
