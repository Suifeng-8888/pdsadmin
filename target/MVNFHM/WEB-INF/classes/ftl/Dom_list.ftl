...
<div class="hniois-table">
    <form action="${packageName}/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${r"${fn:contains(QX,'"}${packageName}${r"/add')}"}">
                ...
                <c:if test="${r"${fn:contains(QX,'"}${packageName}${r"/delete')}"}">
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${r"${fn:contains(QX,'"}${packageName}${r"/update')}"}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            elem: '#table',
            url:'${packageName}/list.do',
            cols: [[
<#list fieldList as e>
                {field:'${e.name}',  title: '${e.comment}'},
</#list>
        ...
        table.on('edit(lay_table)', function(obj){
            if("${r"${fn:contains(QX,'"}${packageName}${r"/update')}"}"=="true"){
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "${packageName}/toEdit.do?id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
