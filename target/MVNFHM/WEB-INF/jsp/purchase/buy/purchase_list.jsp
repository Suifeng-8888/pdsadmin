...
<div class="hniois-table">
    <form action="inquiry/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
    <div class="hniois-table-header">
        <div class="hniois-search-btns">
            ...
            <c:if test="${fn:contains(QX,'inquiry/delete')}">
            ...
            <a id="hniois-add" onclick="goRecord()" class="layui-btn layui-btn-sm layui-bg-green" href="javascript:;">
                <i class="fa fa-lg" aria-hidden="true"></i> 去备案</a>
        </div>
    ...
    <div class="hniois-search-mored layui-anim layui-anim-upbit">
        <div class="hniois-search-body">
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                    <select name="record"  lay-search="" class="hniois-search-data">
                        <option vlaue="">请选择</option>
                        <option vlaue="0">已备案</option>
                        <option vlaue="1">未备案</option>
                    </select>
                </div>
            </div>
        </div>
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'inquiry/update')}">
    <a class="layui-btn layui-btn-xs" lay-event="edit">编辑采购</a>
    </c:if>
</script>
<script type="text/html" id="editBar2">
    <a class="layui-btn layui-btn-xs layui-btn-primary" lay-event="detail">查看</a>
    <c:if test="${fn:contains(QX,'inquiry/delete')}">
        <a class="layui-btn layui-btn-xs layui-btn-danger" lay-event="del">&nbsp&nbsp删&nbsp&nbsp&nbsp&nbsp除&nbsp&nbsp</a>
    </c:if>
</script>
</body>
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
                ...
                {field:'status',  title: '采购状态'},
                {field:'record',  title: '财务备案'},
                {fixed: 'right', align:'center',templet:function (d) {
                        var str = "";
                        if(d.status=="作废"||d.record=="未备案"){
                            str=$('#editBar2').html();
                        }else{
                            str=$('#editBar').html();
                        }
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "inquiry/toEdit.do?id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    function goRecord(){
        ...
        for(var i=0;i<data.length;i++){
            if(data[i].record!="未备案"){
                layui.layer.msg('选择的第'+(i+1)+'个数据已备案');
                return;
            }
            ids.push(data[i].id);
        }
