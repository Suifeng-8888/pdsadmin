...
<div class="hniois-table">
    <form action="keypoint/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${fn:contains(QX,'keypoint/add')}">
                ...
                <c:if test="${fn:contains(QX,'keypoint/delete')}">
                ...
                <a id="hniois-export" onclick="doExport('keypoint/doExport')" class="layui-btn layui-btn-sm layui-bg-orange" href="javascript:;" >
            ...
            <div class="hniois-search-body">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="ctr_name" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">产品批次号</label>
                    <div class="layui-input-block">
                ...
                <div class="layui-form-item">
                    <label class="layui-form-label">监控开始日期</label>
                    <div class="layui-input-block">
                        <input name="ctr_start" class="layui-input hniois-search-data" type="text" autocomplete="off"
                               id="test1"  placeholder="yyyy-MM-dd" >
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">监控结束日期</label>
                    <div class="layui-input-block">
                        <input name="ctr_end" class="layui-input hniois-search-data" type="text" autocomplete="off"
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'keypoint/update')}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            elem: '#table',
            url:'keypoint/list.do',
            cols: [[
                {type:'checkbox'},
                {field:'ctr_id',  title:'序号',align:"center"},
                {field:'im_id', title: '进货序号',align:"center"},
                {field:'pro_name',  title: '产品名称',align:"center"},
                {field:'im_patch', title: '产品批次号',align:"center"},
                {field:'ctr_name',  title: '关键控制点名称',edit:'text',align:"center"},
                {field:'ctr_point', title: '控制点数及单位',edit:'text',align:"center"},
                {field:'ctr_num',  title: '监控次数',edit:'text',align:"center"},
                {field:'ctr_start',  title: '控制开始时间',align:"center"},
                {field:'ctr_end',  title: '控制结束时间',edit:'text',align:"center"},
                {field:'person',  title: '责任人',edit:'text',align:"center"},
                {field:'result',  title: '监控结论',align:"center"},
        ...
        table.on('tool(lay_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                toEdit(data.ctr_id,0)
            }  else if(layEvent === 'edit'){
                toEdit(data.ctr_id,1)
            }else if(layEvent === 'del'){
                layer.confirm('确认删除么', function(index){
                    layer.close(index);
                    obj.del(); //删除对应行（tr）的DOM结构
                    //向服务端发送删除指令
                    doDelete(data.ctr_id);
                });
            }
        });
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "keypoint/toEdit.do?ctr_id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    function doDeletes(){
        ...
        for(var i=0;i<data.length;i++){
            ids.push(data[i].ctr_id);
        }
        layui.layer.confirm('确认删除', function(index){
            $.ajax({
                url: 'keypoint/deletes.do',
                data: {
                    data:ids.toString()
                },
                type: "post",
