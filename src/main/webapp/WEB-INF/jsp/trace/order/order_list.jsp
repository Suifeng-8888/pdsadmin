...
<div class="hniois-table">
    <form action="order/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${fn:contains(QX,'order/add')}">
                ...
                <c:if test="${fn:contains(QX,'order/delete')}">
                ...
                <a id="hniois-export" onclick="doExport('order/doExport')" class="layui-btn layui-btn-sm layui-bg-orange" href="javascript:;" >
                    <i class="fa fa-cloud-download fa-lg" aria-hidden="true"></i> 导出</a>
            </div>
            <div class="hniois-search-inputs">
                <div class="hniois-search-keyword">
                    <input type="text" class="layui-input hniois-search-data" name="order_num" placeholder="订单编号.." />
            ...
            <div class="hniois-search-body">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="sale_name" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">创建日期</label>
                    <div class="layui-input-block">
                        <input name="create_time" class="layui-input" type="text" autocomplete="off"
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'order/update')}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
                {type:'checkbox'},
                {field:'order_num',  title:'订单编号',align:"center"},
                {field:'sale_name', title: '销售商',align:"center"},
                {field:'send_time', title: '发货时间',align:"center"},
                {field:'begin_address', title: '发货地址',align:"center"},
                {field:'recever', title: '收货人',align:"center"},
                {field:'end_address',  title: '目的地',align:"center"},
                {field:'batch_name', title: '产品名称',align:"center"},
                {field:'batch', title: '产品批次号',align:"center"},
                {field:'batch_num', title: '产品数量',align:"center"},
        ...
        table.on('tool(lay_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                toEdit(data.order_num,0)
            }  else if(layEvent === 'edit'){
                toEdit(data.order_num,1)
            }else if(layEvent === 'del'){
                layer.confirm('确认删除么', function(index){
                    layer.close(index);
                    obj.del(); //删除对应行（tr）的DOM结构
                    //向服务端发送删除指令
                    doDelete(data.order_num);
                });
            }
        });
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "order/toEdit.do?order_num="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    function doDeletes(){
        ...
        for(var i=0;i<data.length;i++){
            ids.push(data[i].order_num);
        }
        layui.layer.confirm('确认删除', function(index){
            $.ajax({
                url: 'order/deletes.do',
                data: {
                    data:ids.toString()
                },
                type: "post",
