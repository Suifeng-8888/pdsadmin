    ...
    <form action="user/list.do" method="post" class="layui-form hniois-search-form" id="hniois-search-form">
        <div class="hniois-table-header">
            <div class="hniois-search-btns">
                <c:if test="${fn:contains(QX,'machine/add')}">
                ...
                <c:if test="${fn:contains(QX,'machine/delete')}">
                ...
                <a id="hniois-export" onclick="doExport('machine/doExport')" class="layui-btn layui-btn-sm layui-bg-orange" href="javascript:;" >
                    <i class="fa fa-cloud-download fa-lg" aria-hidden="true"></i> 导出</a>
            </div>
            <div class="hniois-search-inputs">
                <div class="hniois-search-keyword">
                    <input type="text" class="layui-input hniois-search-data" name="mach_name" placeholder="搜索仪器名.." />
            ...
            <div class="hniois-search-body">
                <div class="layui-form-item">
                    ...
                    <div class="layui-input-block">
                        <input type="text" name="mach_id" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">仪器型号</label>
                    <div class="layui-input-block">
                        <input type="text" name="model" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">用途</label>
                    <div class="layui-input-block">
                        <input type="text" name="for_use" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">制造商</label>
                    <div class="layui-input-block">
                        <input type="text" name="factory" autocomplete="off" class="layui-input hniois-search-data">
                    </div>
                </div>
                <div class="layui-form-item">
                    <label class="layui-form-label">设备状况</label>
                    <div class="layui-input-block">
                        <select name="situation" id="hniois-role-select" lay-search="" class="hniois-search-data">
...
<script type="text/html" id="editBar">
    ...
    <c:if test="${fn:contains(QX,'machine/update')}">
...
<script>
    ...
    $(document).ready(function() {
        ...
        data_table = table.render({
            ...
            cols: [[
                {type:'checkbox'},
                {field:'mach_id',  title:'仪器编号',width:200,align:"center"},
                {field:'mach_name', title: '仪器名称',edit:'text',width:200,align:"center"},
                {field:'model', title: '仪器型号',edit:'text',width:200,align:"center"},
                {field:'factory',  title: '制造商',edit:'text',width:200,align:"center"},
                {field:'out_num',  title: '出厂编号',width:200,align:"center"},
                {field:'for_use',  title: '用途',edit:'text',width:250,align:"center",align:"center"},
                {field:'arrive_time',  title: '到达时间',edit:'text',width:150,align:"center"},
                {field:'use_time', title: '启用时间',edit:'text',width:150,align:"center"},
                {field:'address',  title: '存放位置',edit:'text',width:100,align:"center"},
                {field:'situation', title: '设备状况',width:100,align:"center",templet:function(d){
                    var str = d.situation;
                    if(str=="在用"){
                        str =  "<span>在用</span>";
                    }else if(str=="停用"){
                        ...
                        str =  "<span style='color:orchid'>限用</span>"
                    }
                    return str;
                    }},
                {field:'person',  title: '责任人',edit:'text',width:100,align:"center"},
                {field:'bz', title: '备注',edit:'text',width:100,align:"center"},
        ...
        table.on('tool(lay_table)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if(layEvent === 'detail'){
                toEdit(data.mach_id,0)
            }  else if(layEvent === 'edit'){
                toEdit(data.mach_id,1)
            }else if(layEvent === 'del'){
                layer.confirm('确认删除么', function(index){
                    layer.close(index);
                    obj.del(); //删除对应行（tr）的DOM结构
                    //向服务端发送删除指令
                    doDelete(data.mach_id);
                });
            }
        });
    ...
    function toEdit(id,code){
        ...
        layer.open({
            ...
            content: "machine/toEdit.do?mach_id="+id+"&code="+code,
            area: ['650px', '80%']
        });
    }
    ...
    function doDeletes(){
        ...
        for(var i=0;i<data.length;i++){
            ids.push(data[i].mach_id);
        }
        layui.layer.confirm('确认删除', function(index){
            $.ajax({
                url: 'machine/deletes.do',
                data: {
                    data:ids.toString()
                },
                type: "post",
